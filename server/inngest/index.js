...
const sendNewConnectionRequestReminder = inngest.createFunction(
  ...
  <p>
    Click <a href="${process.env.FRONTEND_URL}/connections" style="color:#1b0981;">here</a>
  ...
  <p>Thanks,<br/>PingUp - Stay Connected</p>
    ...
    await step.run("send-connection-request-reminder", async () => {
      ...
      if (connection.status === "accepted") {
        return { message: "Already acceptef" };
      }
...
// iNNGEST FUNCTION TO DELTE THE STORY IN 24 HOURS
...
const deleteStory= inngest.createFunction(
  ...
  async ({event,step})=>{
    ...
    const in24Hours= new Date(Date.now() + 24*60*60*1000)
    await step.sleepUntil('wait-for-24-houres',in24Hours)
